#Admin usernames ('adminajm','qwerty') or ('computer','010101')
import csv
import os
import random
import time
...
def signup() :
    ...
    while True:
        phone=input("Enter your phone number(in form of 1234567890) -> ")
...
def username_checker(username):
    with open('Login Details.csv','r') as ob:
        data=csv.reader(ob)
        if username=='adminajm' or username=='computer' or username=='admin':
            return False
        for i in data:
            if i[0]==username:
                return False
...
def admin(): #For login of Admin
    ...
    username.lower()
    if username!='adminajm' and username!='computer':
    ...
    while True:
        ...
        if cap_count==3:
            ...
            os._exit(1)
        print("You have only",3-cap_count,'chances left')
    if username=='adminajm' and password=='qwerty':
